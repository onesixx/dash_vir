...
def uf_load_model_list():
    #data = pd.read_csv('03.dashui/utils/data/model_list.csv')
    #data = pd.read_csv('./data/model_list.csv')
    data = data[['md_name', 'md_path', 'md_filename', 'md_x_var',
                 'md_y_var', 'md_mae', 'md_mse', 'md_rmse', 'md_desc']]
    return data
...
def uf_save_model_list(dModel):
    ...
    if(len(data.loc[r, :]) < 1):
        ...
        data.loc[r, ['md_path', 'md_filename', 'md_x_var', 'md_y_var', 'md_mae', 'md_mse', 'md_rmse', 'md_desc']] = [dModel['md_path'],
                                                                                                                     dModel['md_filename'], x_str, dModel['md_y_var'], dModel['md_mae'], dModel['md_mse'], dModel['md_rmse'], dModel['md_desc']]

    data = pd.DataFrame(data[['md_name', 'md_path', 'md_filename',
                        'md_x_var', 'md_y_var', 'md_mae', 'md_mse', 'md_rmse', 'md_desc']])
